...
StructuredBuffer<FrameTrans> FrameTransArray : register(t15);
StructuredBuffer<FrameTrans> ChangeFrameTransArray : register(t16);

StructuredBuffer<matrix> OffsetArray : register(t25);

/***********************DO NOT CHANGE VAL'S NAME***********************/

RWStructuredBuffer<matrix> BoneMatrixArray : register(u0);
RWStructuredBuffer<matrix> BoneSocketMatrixArray : register(u1);
RWStructuredBuffer<matrix> InstancingBoneMatrixArray : register(u2);

struct AnimationIfno
{
    int AnimBoneCount;
    ...
    int AnimFrameCount;
    int AnimRowIndex;
    int AnimChangeAnimation;
    float AnimChangeRatio;
    
    
    int AnimChangeFrameCount;
    float3 padding;
    
};


cbuffer AnimationCBuffer 
{
    AnimationIfno animInfo;   
};

[numthreads(256, 1, 1)]
void CSMain(int3 ThreadID : SV_DispatchThreadID)
{
    if (animInfo.AnimBoneCount <= ThreadID.x)
        return;

    float4 ZeroRot = float4(0.f, 0.f, 0.f, 1.f);
    matrix matBone = (matrix) 0.f;

    uint FrameIndex = ThreadID.x * animInfo.AnimFrameCount + animInfo.AnimCurrentFrame;
    uint FrameNextIndex = ThreadID.x * animInfo.AnimFrameCount + animInfo.AnimNextFrame;    
    
    
    float4 Scale = lerp(FrameTransArray[FrameIndex].Scale, FrameTransArray[FrameNextIndex].Scale, animInfo.AnimRatio);
    float4 Pos = lerp(FrameTransArray[FrameIndex].Translate, FrameTransArray[FrameNextIndex].Translate, animInfo.AnimRatio);
    float4 Rot = QuternionLerp(FrameTransArray[FrameIndex].Rot, FrameTransArray[FrameNextIndex].Rot, animInfo.AnimRatio);
    ...
    //Blending
    if (animInfo.AnimChangeAnimation != 0)
    {
        uint ChangeFrameIndex = ThreadID.x * animInfo.AnimChangeFrameCount;

        Scale = lerp(ChangeFrameTransArray[ChangeFrameIndex].Scale, Scale, animInfo.AnimChangeRatio);
        Pos = lerp(ChangeFrameTransArray[ChangeFrameIndex].Translate, Pos, animInfo.AnimChangeRatio);
        Rot = QuternionLerp(ChangeFrameTransArray[ChangeFrameIndex].Rot, Rot, animInfo.AnimChangeRatio);
    }
    
    MatrixAffineTransformation(Scale, ZeroRot, Rot, Pos, matBone);     
    
    
    matrix matOffset = transpose(OffsetArray[ThreadID.x]);
    
    BoneMatrixArray[ThreadID.x] = mul(matOffset, matBone);
    
    BoneSocketMatrixArray[ThreadID.x] = (matBone);
    
    InstancingBoneMatrixArray[animInfo.AnimRowIndex * animInfo.AnimBoneCount + ThreadID.x] = mul(matOffset, matBone);
