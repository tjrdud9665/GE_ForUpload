...
VerticalBoxSlot::VerticalBoxSlot(const VerticalBoxSlot& Other)
	:Super(Other)
...
void VerticalBoxSlot::SetupGeometry(class Renderer* pRenderer)
{
	...
	slotMaxSize.x = SMathUtils::Clamp(Parent->GetMaxDesiredSize().x, 0.0f, parentRenderedSize.x);
	slotMaxSize.y = SMathUtils::Clamp(Parent->GetMaxDesiredSize().y, 0.0f, parentRenderedSize.y / colCnt);
	...
	SVector2 lt = SVector2(-scrSize.x + parentPos.x,
						   (scrSize.y + parentPos.y - (idx)*slotMaxSize.y * 2.0f));
